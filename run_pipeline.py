...
# Step 2: Merge
merge_data(tables).to_csv("input_data/championship_merged.csv", index=False)
merged_df = pd.read_csv("input_data/championship_merged.csv")

# Step 3: Per 90
per90_df = calculate_per90(merged_df)
per90_df.to_csv("input_data/championship_per90.csv", index=False)

# Step 4: Filter by 90s played
filtered_df = filter_by_90s(per90_df, min_90s=4.0)
filtered_df.to_csv("input_data/championship_filtered.csv", index=False)
